...
for filename in os.listdir(folder_path):
    if filename.lower().endswith(extensions):
        ...
        #process_image_lab = image_bgr # cv2.cvtColor(image_bgr, cv2.COLOR_BGR2LAB)

        R = image_bgr[:, :, 2]
        G = image_bgr[:, :, 1]
        
        process_image_lab = np.stack((R, G), axis=2)

        smoothed = cv2.GaussianBlur(process_image_lab, (5, 5), 0)
        ...
        min_area = 500
        area_filtered_contours = [cnt for cnt in contours if cv2.contourArea(cnt) >= min_area]
...
def detect_flakes(self, image, display=False):
    pass